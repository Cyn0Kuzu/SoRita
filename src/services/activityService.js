    ...
    try {
      ...
      if (!currentUser) {
        ...
        if (action === 'app_launched' || action === 'app_state_change') {
          console.log(' [ActivityService] Skipping activity recording - user not authenticated:', action);
          return null;
        }
        console.warn(' [ActivityService] No authenticated user for activity recording:', action);
        return null;
      }
      ...
      if (this.recentActivities.has(activityKey)) {
        ...
        if (now - lastRecorded < this.DUPLICATE_PREVENTION_WINDOW) {
          console.log(' [ActivityService] Skipping duplicate activity:', action);
          return null;
        }
      }
      ...
      if (error.code === 'already-exists') {
        console.log(' [ActivityService] Activity already exists (race condition), skipping:', action);
        return null;
      }

  ...
  static async acceptInvitation(listId, userId, userData) {
    try {
      ...
      try {
        ...
        console.error('⚠️ [CollaborativeListService] Failed to send notification to list owner:', notificationError);
      ...
      for (const collaboratorId of collaborators) {
        if (collaboratorId !== listData.userId) {
          try {
            ...
            console.warn(`⚠️ [CollaborativeListService] Error loading user data for ${collaboratorId}:`, userError);
            // Fallback to stored data
            const collaboratorInfo = collaboratorDetails[collaboratorId];
            if (collaboratorInfo) {
              members.push({
  ...
  static async getAllUserListsSimple(userId) {
    try {
      ...
      try {
        ...
        console.warn('⚠️ [CollaborativeListService] Error getting collaborative lists (will continue with own lists):', collabError);

...
export class UserDataService {
  ...
  static async createUserProfile(userData) {
    try {
      ...
      // Cache locally
      await AsyncStorage.setItem(`userData_${currentUser.uid}`, JSON.stringify(completeUserData));
    ...
    try {
      ...
      if (!userId || userId === auth.currentUser?.uid) {
        try {
          ...
          console.warn('⚠️ [UserDataService] Cache read error:', cacheError);
        }
      }
      ...
      if (!userDoc.exists()) {
        console.warn('⚠️ [UserDataService] User profile not found:', targetUserId);
        return null;
      }
      ...
      if (!userId || userId === auth.currentUser?.uid) {
        ...
        // Update cache
        try {
          await AsyncStorage.setItem(`userData_${targetUserId}`, JSON.stringify(userData));
        } catch (cacheError) {
          console.warn('⚠️ [UserDataService] Cache write error:', cacheError);
        }
      }
      ...
      if (error.code === 'unavailable' || error.message.includes('offline')) {
        ...
        try {
          ...
          console.warn('⚠️ [UserDataService] Cache read error in offline mode:', cacheError);
        }
  ...
  static async updateUserProfile(updates) {
    try {
      ...
      console.log('✏️ [UserDataService] Updating user profile');
      ...
      try {
        ...
        await AsyncStorage.setItem(`userData_${currentUser.uid}`, JSON.stringify(updated));
      } catch (cacheError) {
        console.warn('⚠️ [UserDataService] Cache update error:', cacheError);
      }
      ...
      if (error.code === 'unavailable' || error.message.includes('offline')) {
        ...
        try {
          ...
          if (current) {
            ...
            await AsyncStorage.setItem(`userData_${auth.currentUser.uid}`, JSON.stringify(updated));
            console.log('✅ [UserDataService] Profile updated in cache (offline mode)');
            return true;
          }
        } catch (cacheError) {
          console.warn('⚠️ [UserDataService] Cache update failed in offline mode:', cacheError);
        }

        throw new Error(
          'Şu anda çevrimdışısınız. Değişiklikler internet bağlantısı sağlandığında sync edilecek.'
        );
      }
    ...
    try {
      ...
      if (backupId) {
        ...
        if (backupsSnapshot.empty) {
          console.log('ℹ️ [UserDataService] No backup found for user');
          return false;
        }
  ...
  static async getUserFollows(userId = null) {
    try {
      ...
      console.log(
        `👥 [UserDataService] Retrieved ${following.length} following, ${followers.length} followers`
      );
  ...
  static async clearCache(userId = null) {
    try {
      ...
      console.warn('⚠️ [UserDataService] Error clearing cache:', error);
    }
  }
  ...
  static async updateLastActivity() {
    try {
      ...
      console.warn('⚠️ [UserDataService] Error updating last activity:', error);
    }
  }

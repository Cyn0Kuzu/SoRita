...
export const registerForPushNotifications = async (userId) => {
  try {
    ...
    if (!userId) {
      console.warn(' [PushNotificationService] No userId provided for push notification registration');
      return null;
    }
    ...
    if (Device.isDevice) {
      ...
      if (finalStatus !== 'granted') {
        console.warn(' [PushNotificationService] Permission denied for push notifications');
        return null;
      }
      ...
      try {
        ...
        if (!db) {
          console.warn(' [PushNotificationService] Firebase not initialized, skipping token registration');
          return null;
        }
        ...
        if (Platform.OS === 'ios' || (Platform.OS === 'android' && !__DEV__)) {
          ...
          console.log(' [PushNotificationService] Skipping token generation in development/emulator');
          return null;
        }
      } catch (tokenError) {
      ...
      console.warn(' [PushNotificationService] Must use physical device for Push Notifications');
    }
...
export const sendPushNotification = async (userId, title, message, data = {}) => {
  try {
    ...
    if (!userDoc.exists()) {
      console.warn(' [PushNotificationService] User not found:', userId);
    ...
    if (!pushToken) {
      ...
      if (process.env.NODE_ENV === 'production') {
        console.warn(' [PushNotificationService] No push token found for user:', userId);
      } else {
...
export const sendCommentDeletePushNotification = async (fromUser, toUserId, postTitle) => {
  try {
    console.log(' [PushNotificationService] Sending comment delete push notification...');
...
export const sendLikePushNotification = async (fromUser, toUserId, postTitle) => {
  try {
    console.log(' [PushNotificationService] Sending like push notification...');
  ...
  try {
    ...
    switch (data.type) {
      ...
      case 'comment_deleted':
        console.log(' Comment delete notification received from:', data.fromUserName || 'Unknown user');
        break;
      case 'like':
        console.log(' Like notification received from:', data.fromUserName || 'Unknown user');
        break;

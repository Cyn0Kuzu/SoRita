...
export default function EditMapModal({ visible, onClose, listData }) {
  ...
  const initializeEditMode = async () => {
    ...
    if (listData.places && listData.places.length > 0) {
      ...
      const initializedPlaces = listData.places.map((place) => {
        ...
        console.log(
          `📋 [EditMapModal] Place ${place.name} already has userContent:`,
          place.userContent
        );
        return place;
      });
  ...
  const focusOnPlaces = () => {
    if (editListPlaces.length > 0 && mapRef.current) {
      ...
      const coordinates = editListPlaces
        .map((place) => {
  ...
  const saveChanges = async () => {
    try {
              ...
              if (typeof photo === 'string' && photo.startsWith('file://')) {
                try {
                  ...
                  const downloadUrl = await StorageService.uploadImage(
                    photo,
                    `lists/${listData.id}/places/${place.name}/photos`
                  );
      ...
      Alert.alert('Başarılı', 'Liste güncellendi!', [{ text: 'Tamam', onPress: onClose }]);
    } catch (error) {
    ...
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={onClose}>
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.content}>
          {/* Map */}
          <View style={[styles.mapContainer, placesCollapsed && styles.mapContainerExpanded]}>
            {location ? (
              <MapView
                ref={mapRef}
                style={styles.map}
              ...
              >
                {editListPlaces.map((place, index) => {
                  ...
                  const userColor = isCollaborative
                    ? listData?.colorAssignments?.[addedByUserId] || '#FF6B6B'
                    : '#FF6B6B';
          ...
          {/* Places List */}
          <View
            style={[styles.placesContainer, placesCollapsed && styles.placesContainerCollapsed]}
          >
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={styles.placesTitle}>Mekanlar ({editListPlaces.length})</Text>
              <MaterialIcons
              ...
              >
                {editListPlaces.length > 0 ? (
                  editListPlaces.map((place, index) => {
                    ...
                    const userData =
                      isCollaborative && collaboratorDetails
                        ? {
                            id: addedByUserId,
                            firstName: collaboratorDetails.firstName || '',
                            lastName: collaboratorDetails.lastName || '',
                            displayName:
                              collaboratorDetails.displayName ||
                              `${collaboratorDetails.firstName || ''} ${collaboratorDetails.lastName || ''}`.trim() ||
                              collaboratorDetails.email?.split('@')[0] ||
                              'Kullanıcı',
                            avatar: collaboratorDetails.avatar || '👤',
                            username:
                              collaboratorDetails.username ||
                              collaboratorDetails.email?.split('@')[0] ||
                              '',

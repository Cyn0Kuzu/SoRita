...
const ViewListModal = ({ 
  ...
  useEffect(() => {
    if (visible && listData) {
      console.log(' [ViewListModal] Modal opened for list:', listData.name);
  ...
  const initializeViewMode = async () => {
    ...
    if (listData.places && listData.places.length > 0) {
      ...
      console.log(' [ViewListModal] No places found in listData');
      setListPlaces([]);
    }
  ...
  const getLocationPermission = async () => {
    try {
      ...
      console.log(' [ViewListModal] Location error:', error.message);
      setDefaultLocation();
    }
  };
  ...
  const focusOnPlaces = () => {
    ...
    if (mapRef.current && listPlaces.length > 0) {
      ...
      if (coordinates.length === 0) {
        console.log(' [ViewListModal] No valid coordinates found for focusing');
        return;
      }

      setTimeout(() => {
        if (!mapRef.current) {
          console.log(' [ViewListModal] MapRef lost during timeout');
          return;
        }
      ...
      console.log(' [ViewListModal] Cannot focus - no mapRef or places', {
        hasMapRef: !!mapRef.current,
        placesCount: listPlaces.length
      });
    }
  ...
  const handleFocusPlace = (place) => {
    ...
    if (!latitude || !longitude || !mapRef.current || !mapReady) {
      console.warn(' [ViewListModal] Cannot focus on place - missing coordinates or map not ready', {
          ...
          <View style={styles.mapContainer}>
            {location ? (
              <MapView
                ...
                onMapReady={() => {
                  console.log(' [ViewListModal] Map ready, mapReady:', mapReady, 'listPlaces count:', listPlaces.length);
                  setMapReady(true);
              ...
              >
                {listPlaces.map((place, index) => {
                  ...
                  if (!latitude || !longitude) {
                    console.log(' [ViewListModal] Invalid coordinates for place:', place.name);
                    return null;
                  }
              ...
              {selectedPlaceDetail?.address && (
                <View style={styles.placeDetailInfo}>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <MaterialIcons name="place" size={16} color={colors.textSecondary} style={{ marginRight: 4 }} />

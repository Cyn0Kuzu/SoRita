...
const CollaboratorsModal = ({ visible, onClose, listId, listTitle, isOwner = false }) => {
  ...
  const renderAvatar = (member) => {
    ...
    if (member.role === 'owner') {
      if (member.avatar && ((member.avatar.startsWith('http://') || member.avatar.startsWith('https://')) && member.avatar.length > 10) || member.avatar.startsWith('data:image')) {
        return (
          <View style={styles.ownerAvatarContainer}>
            <Image 
              ...
              resizeMode="cover"
              onError={() => {
                console.log('Owner avatar image failed to load');
              }}
            />
      ...
      return (
        <View style={styles.ownerEmojiAvatar}>
          <Text style={styles.ownerEmojiText}>
            {member.avatar && !member.avatar.startsWith('http') ? (member.avatar || '') : ''}
          </Text>
        </View>
      );
    }
    ...
    if (member.avatar && ((member.avatar.startsWith('http://') || member.avatar.startsWith('https://')) && member.avatar.length > 10) || member.avatar.startsWith('data:image')) {
      return (
        <Image 
          ...
          resizeMode="cover"
          onError={() => {
            console.log('Member avatar image failed to load');
          }}
        />
      );
    }

...
export class PerformanceTimer {
  ...
  mark(label) {
    ...
    if (__DEV__) {
      console.log(`⏱️ [${this.name}] ${label}: ${duration.toFixed(2)}ms`);
    }
    ...
    if (__DEV__) {
      console.log(`⏱️ [${this.name}] Completed: ${totalDuration.toFixed(2)}ms`);
    ...
    promise = new Promise(async (resolve, reject) => {
      ...
      while (attempts < retries) {
        try {
          ...
          const result = await Promise.race([loadFunction(...args), timeoutPromise]);
...
export const getMemoryUsage = () => {
  if (__DEV__) {
    console.log(
      'Memory usage monitoring is only available in browsers with performance.memory API'
    );
  }
  return null;
};
...
export const analyzeBundleSize = () => {
  if (__DEV__) {
    // This feature is web-only and not applicable in React Native.
    console.log('📊 [Performance] Bundle size analysis is not available in React Native.');
  }
};

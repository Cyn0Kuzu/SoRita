  ...
  const handleInvitationResponse = async (notificationId, response) => {
    ...
    try {
      ...
      if (response === 'accepted') {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            console.log(' [NotificationsScreen] User profile not found, using Firebase Auth data');
          }
        } catch (userError) {
          // Offline durumunda veya hata durumunda Firebase Auth'tan bilgileri kullan
          console.warn(' [NotificationsScreen] User data fetch failed, using Firebase Auth data:', userError.message);
          userData = {
            firstName: currentUser.displayName?.split(' ')[0] || 'Kullanıcı',
            lastName: currentUser.displayName?.split(' ')[1] || '',
            displayName: currentUser.displayName || 'Kullanıcı',
      ...
      try {
        ...
        console.warn(' [NotificationsScreen] Failed to update notification status:', notificationUpdateError);
